...
const onBackButtonClick = () => {
  gradeLevelStore.formVisible = false
}
const onSaveEditButtonClick = () => {
  const data = {
    
    name: gradeLevel.value.name,
    year: gradeLevel.value.year
  }

  if (prop.isEdit) {
    data.id = gradeLevel.value.id,
    gradeLevelStore.editGradeLevel(gradeLevel.value.id, data)
  } else {
    gradeLevelStore.createGradeLevel(data)
  }
}

onMounted(async () => {
  if (prop.isEdit) {
    await gradeLevelStore.getGradeLevelById(gradeLevelStore.currItemId)
    gradeLevel.value.id = gradeLevelStore.currentItemObj.id
    gradeLevel.value.name = gradeLevelStore.currentItemObj.name
    gradeLevel.value.year = gradeLevelStore.currentItemObj.year
  }
})
</script>
  ...
  <div class="bg-gray-900 w-1/3 h-full fixed right-0 top-0 z-30 p-4">
    ...
    <form class="w-full m-2">
      ...
      <div class="grid grid-cols-8 mb-2">
        ...
        <input
          v-model="gradeLevel.year"
          type="number"
          class="col-span shadow-sm bg-gray-800 border border-gray-600 text-gray-200 text-sm rounded-r-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
          placeholder="Год"
          required
        />
      </div>
