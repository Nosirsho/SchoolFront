...
import LessonTableItem from './LessonTableItem.vue'

const lessonStore = useLessonStore()
const items = ref([])

const openAddForm = () => {
  lessonStore.formVisible = true
  lessonStore.isEdit = false
}

onMounted(async () => {
  await lessonStore.getLessonsList(false)
  items.value = lessonStore.data
})
</script>
<template>
  <div>
    ...
    <table class="min-w-full">
      ...
      <tbody class="bg-white" v-auto-animate>
        <!--LessonTableItem-->
        <LessonTableItem v-for="item in items" :key="item.id" :id="item.id" :name="item.name" />
        <!--End LessonTableItem-->
      </tbody>
    </table>
  </div>
